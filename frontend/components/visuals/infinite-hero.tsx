...
useFrame((state, delta) => {
  if (meshRef.current) {
    ...
    const time = material.uniforms.u_time.value as number
    material.uniforms.u_time.value = time + delta * 0.3 // throttled speed
...
function ShaderBackground({
  ...
  fragmentShader = `
    ...
    void mainImage( out vec4 fragColor, in vec2 fragCoord )
    {
      vec2 uv = (fragCoord.xy-.5*u_resolution.xy)/u_resolution.y;
        ...
        f *= .9;
        vec3 col = vec3(f * 0.08, f * 0.95, f * 0.58);
  ...
  <Canvas className={className} dpr={[1, 1.5]} gl={{ antialias: false, powerPreference: "high-performance" }}>
...
export default function InfiniteHero() {
  ...
  return (
    <div ref={rootRef} className="relative h-svh w-full overflow-hidden bg-[#002817] text-white">
      <div className="absolute inset-0" ref={bgRef}>
        <ShaderBackground className="h-full w-full" />
      </div>

      <div className="pointer-events-none absolute inset-0 [background:radial-gradient(120%_80%_at_50%_50%,_transparent_40%,_#1a0033_100%)]" />
